...
function arrayContiene(array, elemento) {
  ...
  // Tu código:
  if(array.indexOf(elemento) === true){
    return true;
  } else{
    return false;
  }
...
function numeroMasGrande(numeros) {
  ...
  for(x = 0; x < numeros.length; x++){
    for(y = 0; y < numeros[x].length; y++){
      if(maxInt === undefined || numeros[x][y]>maxInt){
        maxInt = numeros[x][y]; 
      }
    }
  }
...
function todosIguales(arreglo) {
  ...
  function mesesDelAño(array) {
    ...
    if(meses[i] == false && meses[i] == false && meses[i] == false ){
